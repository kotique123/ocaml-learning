...
let describe_int (n : int) : string = failwith "not implemented"

(** [fst_of_triple (a, _, _)] returns the first element of a 3-tuple. *)
let fst_of_triple (t : 'a * 'b * 'c) : 'a = failwith "not implemented"

(** [classify_pair (a, b)] returns "both zero", "first zero", "second zero", or "neither zero". *)
let classify_pair (p : int * int) : string = failwith "not implemented"
...
let is_right_triangle (a : float) (b : float) (c : float) : bool = failwith "not implemented"
