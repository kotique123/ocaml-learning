...
let frequency_map (_lst : int list) : int IntMap.t = failwith "not implemented"
...
let top_n (_n : int) (_map : int IntMap.t) : (int * int) list = failwith "not implemented"

(** Binary search tree *)
type 'a bst = Leaf | Node of 'a bst * 'a * 'a bst

(** [bst_insert x t] inserts [x] into BST [t]. *)
let bst_insert (_x : 'a) (_t : 'a bst) : 'a bst = failwith "not implemented"

(** [bst_mem x t] returns true if [x] is in BST [t]. *)
let bst_mem (_x : 'a) (_t : 'a bst) : bool = failwith "not implemented"

(** [bst_to_sorted_list t] returns all elements of [t] in sorted order (in-order traversal). *)
let bst_to_sorted_list (_t : 'a bst) : 'a list = failwith "not implemented"
...
let anagram_groups (_words : string list) : string list StringMap.t = failwith "not implemented"
