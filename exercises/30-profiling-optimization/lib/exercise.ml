...
let time_it (_f : unit -> 'a) : 'a * float = failwith "not implemented"

(** [sum_list_naive lst] sums a list with fold_right (non-tail-recursive). *)
let sum_list_naive (_lst : int list) : int = failwith "not implemented"

(** [sum_list_fast lst] sums a list with fold_left (tail-recursive, cache-friendly). *)
let sum_list_fast (_lst : int list) : int = failwith "not implemented"

(** [sum_array arr] sums an array with a loop (most cache-friendly). *)
let sum_array (_arr : int array) : int = failwith "not implemented"

(** [float_array_dot a b] computes the dot product of two float arrays efficiently.
    Use a for loop, not Array.fold. *)
let float_array_dot (_a : float array) (_b : float array) : float = failwith "not implemented"

(** [sieve_of_eratosthenes n] returns all primes <= n using a boolean array. *)
let sieve_of_eratosthenes (_n : int) : int list = failwith "not implemented"

(** [merge_sort lst] implements merge sort (returns sorted copy, no mutation). *)
let merge_sort (_lst : int list) : int list = failwith "not implemented"
