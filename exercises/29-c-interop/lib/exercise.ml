...
let create_buffer (n : int) : buffer = failwith "not implemented"

(** [buffer_fill buf v] fills every byte of [buf] with value [v] (0-255). *)
let buffer_fill (buf : buffer) (v : int) : unit = failwith "not implemented"

(** [buffer_sum buf] returns the sum of all bytes in [buf]. *)
let buffer_sum (buf : buffer) : int = failwith "not implemented"

(** [buffer_to_string buf] converts [buf] to a Bytes string. *)
let buffer_to_string (buf : buffer) : bytes = failwith "not implemented"

(** [bytes_to_buffer b] converts Bytes [b] to a buffer. *)
let bytes_to_buffer (b : bytes) : buffer = failwith "not implemented"

(** [xor_buffers a b] returns a new buffer where each byte is a.(i) lxor b.(i).
    Lengths must match. *)
let xor_buffers (a : buffer) (b : buffer) : buffer = failwith "not implemented"
