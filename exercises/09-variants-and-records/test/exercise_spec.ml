...
let person_eq =
  Alcotest.testable
    (fun ppf p -> Format.fprintf ppf "{name=%s; age=%d; email=%s}" p.name p.age p.email)
    (fun a b -> a.name = b.name && a.age = b.age && a.email = b.email)
    ...
    ; ( "Assignment 5: make_person"
      , [ Alcotest.test_case "creates person with correct fields" `Quick (fun () ->
        ...
        ; Alcotest.test_case "creates person with age 0" `Quick (fun () ->
              let p = make_person "Baby" 0 "baby@example.com" in
              Alcotest.(check int) "age 0" 0 p.age)
        ; Alcotest.test_case "creates different persons independently" `Quick (fun () ->
              let p1 = make_person "Alice" 30 "a@example.com" in
              let p2 = make_person "Bob" 25 "b@example.com" in
